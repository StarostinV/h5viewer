...
__all__ = ['ColormapWindow']

SIZE_ = (800, 30)


class ColormapWindow(QtWidgets.QWidget):
    clicked = QtCore.pyqtSignal()

    def __init__(self, range_init, range_whole, title=None):
        self.callback = lambda *x: None
        self.range_init = range_init
        super(ColormapWindow, self).__init__()

        self.setFixedSize(*SIZE_)
        self.title = title or "Colormap"
        self.setWindowTitle(self.title)
        self.range_slider = QRangeSlider(range_whole, title=title, parent=self)
        self.installEventFilter(self)

    def show(self):
        super(ColormapWindow, self).show()
        self.range_slider.setRange(*self.range_init)

    def set_callback(self, callback):
        self.callback = callback

    def eventFilter(self, QObject, event):
        if event.type() in [76, 77]:
            self.callback(self.range_slider.getRange())
        return False
...
class Ui_Form(object):
    def setupUi(self, Form, title=None):
        self.title = title or "QRangeSlider"
        Form.setObjectName(self.title)
        Form.resize(*SIZE_)
...
class Head(Element):
    ...
    def drawText(self, event, qp):
        ...
        qp.drawText(event.rect(), QtCore.Qt.AlignLeft, '%.1f' % self.main.min())
...
class Tail(Element):
    ...
    def drawText(self, event, qp):
        qp.setPen(self.textColor())
        qp.setFont(QtGui.QFont('Arial', 10))
        qp.drawText(event.rect(), QtCore.Qt.AlignRight, '%.1f' % self.main.max())
...
class Handle(Element):
    ...
    def drawText(self, event, qp):
        qp.setPen(self.textColor())
        qp.setFont(QtGui.QFont('Arial', 10))
        qp.drawText(event.rect(), QtCore.Qt.AlignLeft, '%.1f' % self.main.start())
        qp.drawText(event.rect(), QtCore.Qt.AlignRight, '%.1f' % self.main.end())
...
class QRangeSlider(QtWidgets.QWidget, Ui_Form):
    ...
    def __init__(self, range_whole: tuple, title: str = None, parent=None):
        ...
        assert len(range_whole) == 2

        self.setMin(range_whole[0])
        self.setMax(range_whole[1])
...
if __name__ == '__main__':
    ...
    rs = QRangeSlider((0, 100), title='Colormap')
    rs.show()
    rs.setRange(5.5, 10.5)
